...
public class IBeXToViatraPatternTransformation{
	...
	private PatternBody body;
	private Collection<Variable> parameters;
	private HashMap<String, Pattern> viatraPatterns;
	private Set<IQuerySpecification<? extends ViatraQueryMatcher<? extends IPatternMatch>>> viatraSpecifications;
	private HashMap<IBeXNode, Boolean> allNodes;
	...
	private ClassType createClassType(String typeName, EClassifier className) {
		ClassType type = PatternLanguageFactory.eINSTANCE.createClassType();
		type.setTypename(typeName);
		type.setClassname(className);
		return type;
	}	
	private LocalVariable createLocalVariable(String name, Type type) {
	...
	private VariableReference createVariableReference(String var, Variable variable) {
	...
	private Parameter createParameter(String name, Type type, ParameterDirection direction) {
	...
	private ReferenceType createReferenceType(String typename, EStructuralFeature refname) {
	...
	private CompareConstraint createCompareConstraint(CompareFeature operation, ValueReference leftOperand, ValueReference rightOperand ) {
	...
	private LiteralValueReference createLiteralValueReference(Object value) {
	 ...
	 */
	private void iBeXInvocationsToViatraPatternCall(EList<IBeXPatternInvocation>  invocations) {
	...
	private ArrayList<IExpressionEvaluator> IBeXConstraintToViatraConstraint(EList<IBeXAttributeConstraint> constraints) {
...
//					locVarDst_PathExpEXP = createLocalVariable("s", createReferenceType(nodeType_Exp.getName(), ibexConstr.getType()));
	...
	private void iBexEdgeToViatraSchema(EList<IBeXEdge> edges) {
	...
	private List<Constraint> iBeXInjectiveConstraintToViatra(EList<IBeXNodePair> pairs) {
	 ...
	 */
	private Collection<Variable> removeDubsInVariables(Collection<Variable> variables) {
	...
	private IExpressionEvaluator iBeXCSPExpressionEvaluatorBuilder(IBeXCSP csp, List<String> inputParameterNames, int numberOfCsp) {
		return new IExpressionEvaluatorBuilder().iBeXCSPExpressionEvaluatorBuilder(csp, inputParameterNames, numberOfCsp);
	}
	 ...
	 * @param op  
	 */
	private IExpressionEvaluator expressionEvaluatorBuilder(Class<?> javaType, String[] parameters, Object[] values, FunctionType fType, Operator op) {
